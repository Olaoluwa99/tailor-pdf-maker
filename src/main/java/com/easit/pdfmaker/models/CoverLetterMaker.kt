...
class CoverLetterMaker(private val path: String){
    ...
    fun createCoverLetter(
        ...
        when (styleType){
            ...
            StyleType.OMEGA -> createTypeOmega(item, onPdfCreated)
        }
    }
    ...
    private fun createTypeBeta(item: CoverLetterData, onPdfCreated: () -> Unit){
        val document = Document(PageSize.A4)
        val spacing = 10f
        ...
        try {
            ...
            val location = createMain("${item.location}  •  ${item.date}")
            document.add(location)
    ...
    private fun createTypeDelta(item: CoverLetterData, onPdfCreated: () -> Unit){
        val document = Document(PageSize.A4)
        val spacing = 10f
        ...
        try {
            ...
            //Date
            document.add(Paragraph("  "))
            document.add(createMain(item.date))
    ...
    private fun createTypeGamma(item: CoverLetterData, onPdfCreated: () -> Unit){
        val document = Document(PageSize.A4)
        val spacing = 10f
    ...
    private fun createTypeOmega(item: CoverLetterData, onPdfCreated: () -> Unit){
        val document = Document(PageSize.A4)
        val spacing = 10f
        ...
        try {
            ...
            val processedName: String = if (isUppercaseName) item.name.uppercase() else item.name
            val name = createHeader(processedName, 24f, headerFont)
            name.alignment = Element.ALIGN_CENTER
            document.add(name)
